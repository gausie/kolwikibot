...
while (i < MAX_PAGES) {
  ...
  const end = i * 100 + 99;
  const title = `Monsters by number (${start}-${end})`;
  ...
  // Save changes
  await bot.save(title, newText, 'Converted monster table to responsive bynumber format', {
    minor: true,
    bot: true,
  });
